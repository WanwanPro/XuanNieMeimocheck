...
function loadFeishuSdk() {
  ...
  feishuSdkPromise = new Promise((resolve, reject) => {
    ...
    script.async = true;
    script.onload = () => {
      if (window.h5sdk && window.tt) resolve();
      else reject(new Error('飞书 JSAPI 未注入全局对象'));
    };
    script.onerror = () => reject(new Error('飞书 JSAPI 加载失败'));
...
function requestFeishuConfig() {
  ...
  return fetch('/api/auth/feishu/jsapi-config?url=' + encodeURIComponent(pageUrl), { cache: 'no-store' })
    .then(async (r) => {
      const d = await r.json().catch(() => ({}));
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      window.h5sdk.config({
        ...
        jsApiList: config.jsApiList || ['requestAuthCode'],
        onSuccess: () => { /* ready 回调里继续 */ },
        onFail: (err) => finish(reject, new Error((err && err.errMsg) || '飞书 JSAPI 配置失败')),
      });
      window.h5sdk.ready(() => {
        try {
          window.tt.requestAuthCode({
            appId: config.appId,
            success: (res) => finish(resolve, res && (res.code || res.authCode)),
            fail: (err) => finish(reject, new Error((err && err.errMsg) || '免登授权被拒绝')),
          });
        } catch (err) { finish(reject, err); }
      });
    } catch (err) { finish(reject, err); }
    setTimeout(() => finish(reject, new Error('飞书免登超时')), 9000);
  });
}
